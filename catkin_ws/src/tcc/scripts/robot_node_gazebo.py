...
class Robot_ros:
	...
	def back_gr(self):
		...
		clear_bg = rospy.ServiceProxy('/gazebo/reset_world', Empty)
		clear_bg()
...
agent = Agent(gamma=0.99, epsilon=1.0, batch_size=32, 
            n_actions=len(action_space), input_dims=360, lr=0.001)
...
while not rospy.is_shutdown():
	...
	for e in range(epochs):
		state = robot.sensor_read(rospy.wait_for_message('/scan', LaserScan))
		lidar = state 
		...
		while not done:
			...
			new_state = robot.sensor_read(rospy.wait_for_message('/scan', LaserScan))
